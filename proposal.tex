...
\begin{document}
	...
	% ---------- ABSTRACT ----------
	\begin{abstract}
		In this thesis we introduce two procedures for variable selection
		...
		also with multicolinearity. A forward–backward algorithm is also proposed to make feasible these procedures in large data sets. A small
...
\\
	...
	Thus “high-dimensional” datasets are a special case where the number of variables exceeds the number of
	observations, i.e. $n>p$. 
...
%	Dimension reduction techniques (like principal component analysis) will produce linear combinations of the variables which are difficult to interpret unless most of the coefficients of the linear combination are negligent.
	...
	\begin{enumerate}[label=\textbf{\arabic*:}]
		...
		\item To evaluate proposed methods through simulation and application to one or more Cameroon DHS datasets, as well as compare it with standard clustering procedures such as k-mean and hierarchical clustering algorithms.		
	...
	Simulated data will not be used in this study. Performance will be evaluated in terms of classification agreement with the full-variable clustering, reduction in dimensionality, computational cost, and interpretability of the resulting variable subsets.
	...
	\label{ch:lit}
	 ...
	 \section{Background on Clustering and Classification}
	 Cluster analysis (unsupervised classification) seeks to partition data into homogeneous groups:contentReference[oaicite:0]{index=0}.  Classical clustering methods include hierarchical clustering \citep{hartigan1975}, $k$-means \citep{macqueen1967}, and $k$-medoids \citep{kaufman1987}, among others:contentReference[oaicite:1]{index=1}.  Similarly, standard classification rules (supervised learning) include Fisher’s linear discriminant analysis \citep{fisher1936} and classification trees (CART) \citep{breiman1984}:contentReference[oaicite:2]{index=2}.  In many modern applications, the number of observed variables $p$ can be very large relative to sample size $n$.  A general problem in high-dimensional clustering or classification is that many variables may be \emph{irrelevant} (pure noise) or \emph{redundant} (multicollinear), so that the same cluster structure could be recovered from a smaller subset of variables:contentReference[oaicite:3]{index=3}.  Fraiman \emph{et al.} (2008) emphasize that in cluster problems “the amount of variables ... is too high” due to noisy non-informative or strongly correlated features, and the true signal can be extracted by selecting a reduced subset:contentReference[oaicite:4]{index=4}.  Thus variable selection is critical for both interpretability and clustering accuracy in high-dimensional data.
	 
	 \section{Variable Selection Approaches}
	 Variable selection for clustering can be broadly categorized by how they define and find “relevant” features.  In supervised learning, feature selection has been studied extensively \citep{blum1997selection,guyon2003}.  For clustering, one must first define relevance: in a model-based (mixture) view, relevant variables are those whose distribution depends on the latent cluster label, whereas irrelevant variables are independent of the clusters and convey no clustering information:contentReference[oaicite:5]{index=5}.  Redundant variables, which are correlated with relevant ones, are conditionally uninformative given the relevant set but can still help if relevant features were absent:contentReference[oaicite:6]{index=6}. 
	 
	 Existing methods can be classified as {\it filter}, {\it wrapper}, or {\it embedded} approaches.  Filter methods score or filter variables before clustering (e.g.\ by variance or feature scoring) and then cluster on the selected set, whereas wrapper methods search for subsets by repeatedly applying the clustering algorithm and measuring fit \citep{blum1997selection}.  Wrapper approaches often achieve better results at the cost of higher computation \citep{blum1997selection,guyon2003}.  In fact, as a recent survey notes, “wrapper methods have recently attracted most of the attention and are the most widespread” in model-based clustering, since they tend to yield better classification and modeling accuracy:contentReference[oaicite:7]{index=7}.  For example, Blum and Langley (1997) observed that wrapper methods typically outperform simple filters:contentReference[oaicite:8]{index=8}. 
	 
	 Within wrappers for mixture-model clustering, methods include Bayesian variable-selection (using latent indicators and MCMC) \citep{raftery2006variable,tadesse2005bayesian}, penalization (sparse likelihood or lasso penalties) \citep{witten2010framework,maag2012penalized}, and model-selection approaches \citep{raftery2006variable}.  For instance, Raftery and Dean (2006) recast selection as a model-selection problem using BIC for Gaussian mixtures, jointly choosing variables, cluster number and model by approximate Bayes factors.  Witten and Tibshirani (2010) proposed a sparse $k$-means framework that penalizes feature weights to select relevant variables for clustering.  Fop and Murphy (2018) provide a detailed review of these model-based selection methods, noting that the goal is always to retain only the variables containing “essential clustering information” and to discard irrelevant noise:contentReference[oaicite:9]{index=9}. 
	 
	 \section{Fraiman \emph{et al.} (2008) Methodology}
	 %	Fraiman \emph{et al.} (2008) introduce two novel wrapper methods specifically for partitioning procedures (both clustering and classifiers):contentReference[oaicite:10]{index=10}.  The key idea is to “blind” variables by replacing their values with uninformative estimates, and then measure how well the original partition is preserved.  Formally, let $f(\cdot)$ be the partition rule (cluster assignment) on $\mathbb{R}^p$.  For a subset of indices $I\subset\{1,\dots,p\}$, define a “blinded” data vector $Y^I$ by $Y^I_i = X_i$ if $i\in I$, and $Y^I_i=E(X_i)$ otherwise (here $E(X_i)$ is a location constant, e.g.\ the marginal mean or median).  They then define an objective function 
	 \[
	 h(I) \;=\; \sum_{k=1}^K P\bigl(f(X)=k,\;f(Y^I)=k\bigr),
	 \]
	 i.e.\ the probability that a point is assigned to the same cluster before and after blinding variables outside $I$:contentReference[oaicite:11]{index=11}.  Intuitively, $h(I)$ measures the fraction of data whose cluster label is unchanged when only features in $I$ are kept.  One seeks the subset $I$ of a given size $d$ that maximizes $h(I)$ (or equivalently minimizes the loss of classification).  In practice, given sample data, Fraiman \emph{et al.} define the analogous empirical objective $h_n(I)$ (essentially the proportion of points whose labels coincide under the reduced data) and choose the subset that maximizes it.
	 
	 The two procedures differ in how the variables outside $I$ are blinded.  In the \emph{marginal-mean} approach, each unused feature is set to its marginal mean (or trimmed mean, median, etc.):contentReference[oaicite:12]{index=12}.  This detects “noisy” noninformative variables: if a variable truly carries no clustering signal, setting it to a constant should not affect cluster labels, and leaving it out will not decrease $h(I)$.  In the \emph{conditional-mean} approach, variables are replaced by their conditional expectation given $X_I$.  Concretely, for each data point, the $j$th coordinate is replaced by the mean of its $r$ nearest neighbors (in $X_I$-space) among the sample (or, robustly, the local median):contentReference[oaicite:13]{index=13}.  This way the blinded variables reflect only the signal explained by the chosen subset, effectively removing redundant variation.  The conditional method is more powerful (it can handle multicollinearity) but requires larger samples and heavier computation due to nonparametric estimation:contentReference[oaicite:14]{index=14}.  
	 
	 Fraiman \emph{et al.} provide strong consistency results: as $n\to\infty$, the empirical maximizer $I_n$ converges to the optimal set $I_0$ for both the mean and conditional methods (under mild regularity):contentReference[oaicite:15]{index=15}.  They note that an exhaustive search over all subsets is feasible only for small $p$; for larger dimensions they propose a heuristic forward–backward algorithm.  This algorithm starts with the single variable whose removal most degrades the clustering (i.e.\ the variable with minimum $h_n$ when blinded), then iteratively adds variables that yield the largest increase in $h_n$ until a target classification efficiency is reached (forward phase), and finally attempts to remove any redundant variables one by one (backward phase).  In practice the forward-backward search often found the same optimal subsets as exhaustive search in the examples tested, but at a fraction of the computational cost:contentReference[oaicite:18]{index=18}.
	 
	 \section{Empirical Results (Fraiman \emph{et al.} 2008)}
	 Fraiman \emph{et al.} illustrate their methods on both moderate and high-dimensional data.  In one example, 98 schools in Buenos Aires were clustered (by $k$-means) into 3 groups based on 15 survey items (teachers’ and headmasters’ responses).  They then applied variable selection (exhaustively, since $p=15$) and found that only 8 variables were needed to completely recover the 3 clusters ($100\%$ accuracy):contentReference[oaicite:19]{index=19}.  These variables included headmasters’ evaluations of school knowledge (V3, V4) and relationships with parents and students (V7, V8), as well as teachers’ evaluations of building conservation, didactic resources, and relationships (V11, V12, V14, V15):contentReference[oaicite:20]{index=20}.  Relaxing the accuracy to $95\%$ allowed subsets of size 6, and at $92\%$ accuracy only 4 variables sufficed (e.g. \{V4, V7, V11, V14\}).  The forward–backward algorithm recovered essentially the same variable sets as the exhaustive search in these trials, confirming its effectiveness:contentReference[oaicite:21]{index=21}:contentReference[oaicite:22]{index=22}.  The conditional-mean method yielded a similar set, often eliminating one additional redundant feature (in this case, a student-relationships item):contentReference[oaicite:23]{index=23}.
	 
	 In another example, they analyzed electricity consumption curves (96 time-intervals) for 101 households, clustered into 2 types (plus outliers) via a trimmed $k$-means.  Here $p=96$ is large, so they ran the forward–backward search.  Table 4 of Fraiman \emph{et al.} reports, for efficiency thresholds of 90\%, 95\%, and 100\%, the minimal number of time-intervals needed.  Using the simple mean-blanking method required 15, 22, and 33 variables respectively, whereas using the conditional-mean method (with 5-nearest neighbors) required only 4, 6, and 9 variables:contentReference[oaicite:24]{index=24}.  Thus in this high-dimensional example the conditional strategy dramatically reduced the feature set while preserving cluster accuracy.  The most informative intervals were concentrated in certain time windows (e.g. early morning and evening), consistent with the interpretation that the two consumer types differ in peak-hour usage (see Fig. 5 in Fraiman \emph{et al.}):contentReference[oaicite:25]{index=25}.
	 
	 \section{Comparison to Other Work}
	 Fraiman \emph{et al.}’s approach is a model-free, geometric wrapper that focuses on preserving cluster labels.  In contrast, much of the recent literature has focused on model-based criteria or penalization.  For example, Dean and Raftery (2006) integrate variable selection into Gaussian mixtures using a BIC comparison, automatically determining the number of clusters and feature set.  Witten and Tibshirani (2010) and related sparse clustering methods use regularization (Lasso penalties) to shrink uninformative features toward zero weight during clustering.  These methods also demonstrate that removing irrelevant dimensions often improves cluster recovery (for instance, Raftery and Dean found higher clustering accuracy without noise features).  The statistical survey by Fop and Murphy (2018) reviews these methods and notes that in model-based clustering, relevant variables are those whose distribution \emph{depends} on cluster membership:contentReference[oaicite:26]{index=26} – the same idea underlying Fraiman \emph{et al.}’s criterion of label stability:contentReference[oaicite:27]{index=27}:contentReference[oaicite:28]{index=28}.
	 
	 Empirically, many authors have applied variable selection to real data (e.g.\ microarray gene clustering, customer segmentation, etc.).  While Fraiman \emph{et al.} focused on these two examples, other studies (e.g.\ feature selection for gene expression clustering) report similar findings: only a small subset of variables is truly needed to reveal the latent groups.  However, methods vary widely: some use filter statistics (variance, mutual information), others use wrapper criteria (e.g.\ change in $k$-means objective when omitting a variable), and still others use embedded criteria (entropy, log-likelihood).  Broadly, the consensus is that proper variable selection can greatly simplify cluster interpretation and improve performance, as also noted by Fraiman \emph{et al.} and by others:contentReference[oaicite:29]{index=29}:contentReference[oaicite:30]{index=30}. 
	 
	 \section{Discussion and Research Gaps}
	 Fraiman \emph{et al.} provide a clear and consistent framework for variable selection in clustering, with solid theoretical backing (strong consistency) and practical algorithms.  They highlight the importance of distinguishing between purely noisy features and redundant ones (multicollinearity), which many simpler methods ignore.  The empirical results demonstrate substantial dimension reduction without loss of clustering accuracy. 
	 
	 Nonetheless, several open challenges remain.  As the authors note, their forward–backward search becomes computationally expensive as $p$ grows, suggesting the need for more scalable search heuristics or optimization techniques:contentReference[oaicite:31]{index=31}.  In ultra-high dimensions ($p\gg n$), additional regularization or screening steps may be needed.  Moreover, their approach assumes a fixed number of clusters and a fixed clustering method; in practice the cluster number may be unknown and intertwined with feature choice (as tackled by Bayesian or BIC-based methods).  Extensions to mixed data types, streaming data, and complex cluster shapes are also largely unexplored.  Finally, while Fraiman \emph{et al.} measure success by label stability, other criteria (cluster compactness, separation indices) could be considered and compared.  
	 
	 In summary, Fraiman \emph{et al.} (2008) make a significant contribution by formalizing “feature blinding” for cluster variable selection and proving its consistency.  Their work builds on and complements both classical ideas (e.g.\ Fowlkes \emph{et al.} (1988) who also proposed reducing variables for clustering) and modern developments (model-based and penalized methods).  Future research could address the computational limitations and integrate these ideas with automatic model selection for clustering.  Overall, this line of work remains important for uncovering interpretable structures in high-dimensional unsupervised learning problems. 
